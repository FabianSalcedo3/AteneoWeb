...
public class ContattiRepositoryImpl implements ContattiRepository {
	...
	@Override
	public Contatti findByDomicilio(String domicilio) {
		Contatti contatti = null;
		EntityManager em = null;
		EntityTransaction tx = null;
		try {
			...
			tx = em.getTransaction();
			tx.begin();
			contatti = em.createQuery("from Contatti where domicilio = :domicilio", Contatti.class).setParameter("domicilio", domicilio).getSingleResult();
			tx.commit();
		} catch (Exception e) {
			System.err.println(e.getMessage());
			if (tx != null && tx.isActive())
